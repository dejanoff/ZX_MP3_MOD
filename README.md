...
[![Version](https://img.shields.io/badge/version-1.2.0-blue.svg)](https://github.com/dejanoff/ZX_MP3_MOD/releases/tag/v1.2.0)
...
👉 **[Скачать MP3toMOD.exe (Windows 64-bit, v1.2.0)](https://github.com/dejanoff/ZX_MP3_MOD/releases/download/v1.2.0/MP3toMOD.exe)** *(~11 МБ)*  
...
### 📂 4. Раздельное запоминание папок, авто-именование и нормализация путей
- Входная папка с музыкой и выходная папка экспорта MOD запоминаются раздельно в `config.json`.
- Пути к файлам всегда автоматически нормализуются к стандартным системным разделителям Windows (`\`).
...
### 🏷️ 5. Редактируемое название трека (Song Title)
- Название трека прописывается в заголовок файла ProTracker MOD (до 20 символов) и отображается в плеере (например, в **Wild Player** на ZX Spectrum).
- Заполняется автоматически из имени исходного файла, но доступно для любого ручного редактирования прямо в интерфейсе.
- Кириллические названия безопасно транслитерируются в чистый ASCII.

### 🎚️ 6. Интерактивные ползунки стартовой позиции и длительности
- Удобный выбор точки старта (**Start Position**) и длительности (**Duration**) как вручную с клавиатуры (`MM:SS` или секунды), так и плавным перемещением ползунка мышью.
- Диапазоны ползунков автоматически подстраиваются под реальную длительность загруженного аудиофайла.

### 🛡 7. Встроенная бинарная валидация
...
3. **Song Title:** отредактируйте название трека (до 20 символов, отображается в плеере).
4. **Start Position:** время начала трека (ввод с клавиатуры `MM:SS` / секунды или перемещение ползунка мышью).
5. **Duration:** длительность фрагмента в секундах (ввод с клавиатуры или перемещение ползунка мышью).
6. **Quality:** выберите качество (по умолчанию `OPTIMAL 9309 Hz`).
7. **GS Memory:** целевой объем памяти General Sound (`1 MB`, `512 KB`, `2 MB`, `4 MB`, `Unlimited`).
8. **Output Mode:**
...
9. **Stereo Downmix:** выберите `Mix Stereo (0.5*L + 0.5*R) -> Mono`.
10. Нажмите **[ Analyze Settings ]** для предварительной проверки параметров.
11. Нажмите **[ Convert to MOD ]** для выполнения конвертации.
...
Проект полностью покрыт модульными тестами (32 теста):
```bash
...
- Работу графического интерфейса: синхронизацию интерактивных ползунков (Scale), лимит и транслитерацию названия трека, нормализацию системных путей.
