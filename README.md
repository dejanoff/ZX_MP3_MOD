...
- **Нахлёст и кроссфейд (1 строка = 120 мс):** каждый сэмпл нарезается с запасом в 1 дополнительную строку ($64 + 1 = 65$ строк). Хвост уходящего сэмпла плавно затухает ($1.0 \to 0.0$), а голова нового сэмпла плавно нарастает ($0.0 \to 1.0$). Сумма громкостей всегда строго равна $1.0$.
- **Автоматизация команд громкости (`C40` и `C00`):** новый сэмпл запускается на строке 0 на полной громкости (`C40`), а на строке 1 предыдущий канал моментально глушится (`C00`), разгружая микшер General Sound.
